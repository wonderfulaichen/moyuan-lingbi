...
export const MessageBubble: React.FC<MessageBubbleProps> = ({
  ...
  isCollapsed, isLastAssistant, copyToast, onCopy, onEdit, onRegenerate,
  onToggleCollapse, isLongContent, isProcessing, messageRef,
}) => {
        ...
        {isEditing ? (
          <div className="rounded-xl overflow-hidden" style={{ border: '1px solid var(--color-primary-400)' }}>
            ...
            <div className="flex justify-end gap-1.5 px-2 py-1.5" style={{ borderTop: '1px solid var(--color-border-default)', backgroundColor: 'var(--color-surface-muted)' }}>
              <button onClick={() => onEdit(msg.id, msg.content)} className="text-[10px] px-2 py-1 rounded-md border-none cursor-pointer" style={{ color: 'var(--color-text-muted)' }}>取消</button>
          ...
          <>
            <div className="rounded-xl px-3 py-2 text-[11px] leading-relaxed whitespace-pre-wrap break-words relative"
              ...
              <div className={`flex items-center gap-0.5 mt-1 pt-1 ${msg.role === 'user' ? 'justify-start' : 'justify-end'}`}
                style={{ borderTop: '1px solid var(--color-border-default)', opacity: 0.7 }}>
                {msg.role === 'user' && (
                  <>
                    {copyToast === msg.id ? (
                      <span className="text-[9px] px-1.5 py-0.5 rounded-md whitespace-nowrap"
                        style={{ background: 'var(--color-emerald-500)', color: '#fff' }}>✓ 已复制</span>
                    ) : (
                      <button onClick={(e) => { e.stopPropagation(); onCopy(msg.content, msg.id); }}
                        title="复制"
