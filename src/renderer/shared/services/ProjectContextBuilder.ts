...
class ProjectContextBuilder {
  buildForTarget(target: ContextTarget, options: Partial<ContextOptions> = {}): string {
    const opts = { ...DEFAULT_OPTIONS, ...options };
    const project = dataService.getActiveProject();
    const sections: string[] = [];

    sections.push(this.buildProjectHeader(project));
    sections.push(this.buildInspiration(project, opts));
    sections.push(this.buildScheme(project, opts));

    if (target === 'character') {
      sections.push(this.buildCharacterContext(project, opts));
    } else if (target === 'world') {
      sections.push(this.buildWorldContext(project, opts));
    } else if (target === 'timeline') {
      sections.push(this.buildTimelineContext(project, opts));
    } else if (target === 'outline' || target === 'chapter') {
      sections.push(this.buildOutlineContext(project, opts));
    }

    sections.push(this.buildOutline(project, opts));
    sections.push(this.buildProgress(project, opts));

    return sections.filter(s => s.length > 0).join('\n\n');
  }

  buildFullContext(options: Partial<ContextOptions> = {}): string {
    const opts = { ...DEFAULT_OPTIONS, ...options };
    const project = dataService.getActiveProject();
    const sections: string[] = [];
  ...
  detectTarget(userText: string): ContextTarget {
  ...
  private buildProjectHeader(project: ProjectMeta | null): string {
  ...
  private buildInspiration(project: ProjectMeta | null, opts: ContextOptions): string {
    if (!opts.includeInspiration || !project?.inspiration?.text?.trim()) return '';
    return `## 💡 灵感来源\n${project.inspiration.text.slice(0, 500)}`;
  }

  private buildScheme(project: ProjectMeta | null, opts: ContextOptions): string {
  ...
  private buildCharacterContext(project: ProjectMeta | null, opts: ContextOptions): string {
    if (!opts.includeExistingContent) return '';
    const existing = this.getFilesByFolderType('characters', opts.maxExistingFiles);
    if (existing.length === 0) return '## 📋 已有角色\n（暂无角色设定）';

    const parts = ['## 📋 已有角色（生成新角色时必须参考，避免重复或矛盾）'];
    for (const f of existing) {
      const roleType = this.extractRoleType(f.content);
  ...
  private buildWorldContext(project: ProjectMeta | null, opts: ContextOptions): string {
    if (!opts.includeExistingContent) return '';
    const existing = this.getFilesByFolderType('world', opts.maxExistingFiles);
  ...
  private buildTimelineContext(project: ProjectMeta | null, opts: ContextOptions): string {
    if (!opts.includeExistingContent) return '';
    const existing = this.getFilesByFolderType('timeline', opts.maxExistingFiles);
  ...
  private buildOutlineContext(project: ProjectMeta | null, opts: ContextOptions): string {
    if (!opts.includeExistingContent) return '';
    const outlineFiles = this.getFilesByFolderType('outline', opts.maxExistingFiles);
    const chapterFiles = this.getFilesByFolderType('chapters', opts.maxExistingFiles);
  ...
  private buildOutline(project: ProjectMeta | null, opts: ContextOptions): string {
  ...
  private buildProgress(project: ProjectMeta | null, opts: ContextOptions): string {
  ...
  private getFilesByFolderType(folderType: string, maxFiles: number): VFile[] {
  ...
  private extractRoleType(content: string | undefined): string {
    if (!content) return '';
